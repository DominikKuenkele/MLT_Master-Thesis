...
In the next step, it is tested if the agents can learn to extract features of the objects together.
...
This follows the approach of the \emph{RE generators} described in Section \ref{sec:referring_expression_generation}.
...
This is then reduced to $LSTM_o=1500$ dimensions based on the results of the previous experiments, and it is used as the initial state of the captioning LSTM.
...
This is then passed to a linear layer that produces a vector with 13 dimensions, corresponding to the 2 sizes, 3 shapes, and 8 colors.
...
If configurations perform better on the other hand, it means that the agents learned to encode some meaning in their messages.
While for both 'Dale' datasets several configuration diverge from the baseline, in other words, learning takes place, all learning curves align with the baseline for the 'CLEVR color' dataset and are therefore left out.
...
Fast learning takes place in the first 10.000 to 15.000 games, and it decelerates afterwards.
...
It already produces the whole correct referring expressions in 62,66\% of the samples, and it achieves an \emph{F1-score} of 81,36\%.
...
For the 'Dale' dataset, the \emph{shape} is identified well while the agents struggle more with the \emph{color} and the most with the \emph{size}.
The agents identify the \emph{shape} and the \emph{size} perfectly on the 'CLEVR color' dataset, but they only predict few \emph{colors}.
...
The differences between the two 'Dale' datasets might be explainable by the number of scenes in which the color is part of the referring expression.
...
This results in a random guess of one out of the eight \emph{colors}, explaining the \emph{overall accuracy} of $\frac{1}{8}=12,5\%$.

The final attribute is the \emph{size}.
While the agents easily learn that the \emph{size} is never part of the referring expression for the 'CLEVR color' dataset, they struggle with the 'Dale' datasets.
...
Especially, the agents predict too short referring expressions as the precision of the \emph{<pad>} token is lower than its recall.
...
The reason might be the same.
The \emph{size} attribute is more often needed to discriminate the target object and appears therefore in more samples.
Therefore, it is more central in the referring expressions.
...
\begin{figure}[ht!]
    ...
    \caption{Learning curves of all attribute-predictor games on the 'Dale' datasets. The graphs show the accuracy of the \emph{shape}, since it highlights the learning effect best. In the left column, the colors correspond to different vocabulary sizes $|V|$ while in the right column, the colors correspond to different message lengths $n$. The baseline is marked in black.}
...
However, there are configurations with $|V| \in \{16,10\}$ that need 15.000 to 20.000 games, but they outperform the configurations which learned faster.
...
One configuration even reaches over 95\% after 15.000 games, but then stagnates until the cutoff.
...
The \emph{size} however corresponds to a random guess on the 'Dale' dataset and only achieves 76,72\% on the 'CLEVR color' dataset.
...
When only predicting multi-hot vectors, much fewer layers and weights that need to be trained are involved, and hence the models can learn the given task.
...
Interestingly, when the agents can only use one token to encode their message ($|V|=2$), an improvement of predictions of the \emph{shape} comes at the expense of the accuracy of the \emph{color} and vice versa.
...
The baseline accuracies of the \emph{shape} and \emph{color} are lower, but they can be improved to similar peaks as for the 'Dale-2' dataset.
The highest accuracy for the \emph{shape} lies at 97,18\%, while it lies at 95,23\% for the \emph{color}.
This indicates that the larger number of distractors makes it difficult to focus on the correct object, and instead it describes distractors when the receiver is on their own.
But when the sender can communicate the correct object, the receiver describes the attributes as well as it does with fewer distractors.
...
The fact that the sender has a smaller influence on the 'CLEVR color' dataset than on the 'Dale' dataset is surprising since the sender would need to communicate only the \emph{color} instead of additionally the \emph{shape} (and \emph{size}).
...
While for the 'Dale' datasets the receiver has a choice between only 2 to 5 objects, they need to decide between up to 10 objects on the 'CLEVR color' dataset.
That might confuse the model about where to focus on and subsequently how to connect it to the one-hot vector.
Future experiments with a similar dataset including fewer distractors would be needed to verify this hypothesis.
...
However, the communication is successful very often as it helps the receiver to identify the correct attributes and referring expression.