...
On the 'Dale-2' dataset, one can see that most of the emerged languages align to English in some way.
The least aligned language is $Lang_{1,100}$ with 86,13\% towards the salience order 'Sh > Si > C'.
...
With $n=2$, larger vocabulary sizes can bring the loss below 80\% as seen for the languages $Lang_{2,50}$ and $Lang_{2,100}$.
Languages with $n \geq 3$ are aligned more, whereas languages with $n=3$ perform the best.
Looking at the vocabulary size, one can see that again a vocabulary size of $|V|=2$ render the emerged language more different from English.
...
On the 'Dale-5' dataset, much fewer languages emerge and those that do emerge have a much lower alignment to English referring expressions than languages on the 'Dale-2' dataset.
...
The \emph{color} plays a larger role than on the 'Dale-2' dataset, but it is still the least important attribute.
...
It suggests that the agents make use of other underlying structural patterns rather than the defined attributes to communicate and solve the task.
