...
The languages that emerge are analyzed quantitatively using a probing approach.
Probing can be used to analyze and interpret the hidden representations in neural networks \citep{Conneau2018}.
Hereby, a second neural model is trained to classify selected linguistic features on the basis of the hidden representations.
The model is expected to be able to easily fit the linguistic features it classifies for.
This is considered evidence that these features are expressed in the underlying hidden representations of the main model.
In the case of the emerged language, a simple encoder-decoder model is trained to translate the world of the agents, expressed by their messages, into English referring expressions, based on the GRE algorithm by \citet{Dale1995}.
Recall that this incremental algorithm is based on an order in salience for the attributes.
...
For this reason, the emerged languages are compared to all six possible salience orders, given the three attributes \emph{shape}, \emph{color} and \emph{size}.
...
The dimension of the linear layer is kept small to prevent memorizing the dataset and to force the model to find abstract connections between the input and the target.
...
Since the probing task only focuses on how well the given messages of the agents can be related to English referring expressions, no dropout is applied, and the model is trained and validated on the complete dataset; no test or validation split is used.
The models are trained for 200 epochs with a learning rate of $lr=0,004$ and a batch size of 32 samples per batch.

The translation model can learn two characteristics with this setup.
First, it can learn to find alignments between the emerged language and the English referring expressions, which is the goal of the probing.
...
This second characteristic leads to a low loss during training, even though there are no connections to the emerged language.
In this test, only the first characteristic is interesting and shows the alignment between the emerged language and English referring expressions.
For that reason, two reference baselines are calculated for each salience order of the attributes, so to eliminate the influence of the second characteristic.
% SD: Baselines?
% DK: (done)
...
This verifies that the model is able to learn an abstract representation from the input and the resulting loss should be close to zero (since the input is perfectly aligned with the target).
...
The lower $L_{norm}$, the higher the alignment between the emerged language and English referring expressions.
