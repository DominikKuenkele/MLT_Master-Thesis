...
\label{sec:langauge-games:reference-resolution}
\subsubsection*{Setup}
In the final experiments, the agents are tasked to produce and understand referring expressions only based on the implicit human knowledge present in the scenes, as for example the attributes of the objects.
...
By this, the agents can't just reuse these human referring expressions, but they need to learn to generate them themselves during the language games only based on the visual input.
...
As in Section \ref{sec:reference_resolution}, the agents are trained on the 'Dale-2', 'Dale-5' and 'CLEVR color' datasets to test if the agents are able to first communicate a target object and secondly describe the target object discriminatively with a small vocabulary.
...
Figure \ref{fig:learning-curves_coordinate-predictor} shows the learning curves for the \textbf{coordinate reference resolver}.
As visible, all configurations perform exactly as the baselines across all datasets.
The baselines show the predictions of the receiver without information from the sender, and they correspond to the structural bias in the dataset.
This shows that the agents don't communicate successfully in any of the configurations to solve the task by predicting correct coordinates.
...
An increase in the accuracy with a stable mean distance shows a behavior where the agents make more correct predictions on the target objects, while the wrong predictions are further away from the target object.
...
The accuracy of the baseline is already lower to begin with, and a sender can lift this score in almost all configurations.
But as for the other datasets, the final accuracy is still low, and it reaches a maximum of only 25\%.
...
As expected, the task is solved better by the agents, and they communicate successfully across all datasets.
...
This indicates that one symbol is too few to encode all meaning, but too many symbols are too difficult to learn.
...
Looking at the results on the 'Dale-5' dataset (see Figure \ref{fig:visualizations_game_dale-5_attention}), the model usually focuses on several objects at once.
