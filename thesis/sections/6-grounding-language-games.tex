...
This yields 2.048 interactions that are used to analyze the emerged language.

Across all experiments, the same restrictions on the vocabulary and the produced messages are compared.
...
The vocabulary with 16 symbols corresponds to the 13 attributes the objects can have and it aligns with human language referring expressions.
However, a smaller vocabulary size increases the pressure to converge on meaning, which is why also 10 and 2 symbols are tested.
On the other side, the agents might learn to assign one symbol with every combination of attributes instead of a compositional language.
