...
One can then expect the agents to align their own language with the target referring expressions, and a high alignment with the salience order 'Sh > C > Si' is expected.
Looking at the results, this is partially confirmed for the 'Dale-2' dataset.
Some emerged languages reach one of the highest alignments to English referring expressions  with a normalized loss of 49,22\% across all emerged languages in this thesis.
However, the salience order with the highest alignment is 'Sh > Si > C', and it values the \emph{size} more than the \emph{color}, instead of the reversed way as in the training data of the agents.
...
The receiver therefore seems to make use of the sender's ability to extract the \emph{shape} from the images and it reproduces what the sender communicated.
However, without a deeper analysis, it can't be ruled out that the receiver extracts the \emph{shape} themselves from the image, and it uses the sender's message only to determine the correct target object in the scene.
...
Looking at the influence of the message length, one can see that the language is the least aligned to the salience order 'Sh > Si > C' with $n=1$.
...
Hereby, languages with $n \in \{3,4\}$ have the highest alignment to English.
Languages with the highest alignment also mostly make use of a medium-sized vocabulary of $|V| \in \{10,16\}$ as for example $Lang_{2,10}$, $Lang_{3,16}$, $Lang_{4,16}$ and $Lang_{6,10}$.
While larger vocabularies can still align relatively much with English referring expressions, languages with $|V|=2$ are much less similar.
...
While the loss for the salience order 'Sh > Si > C' is lower than 70\% for two languages, it stays between 70\% and 80\% for most languages.
...
While the receiver achieves high precision and recall scores on each attribute (recall Tables \ref{tab:results:re-generator-game_size-shape} and \ref{tab:results:re-generator-game_color}), each attribute also plays a bigger role in the message by the sender.
Indeed, one can see an alignment between the predictions of the receiver and the usage of attributes in emerged language.
...
As for the previous analyses, a vocabulary size of $|V|=2$ seems to be too small for the agents to use similar referring expressions as in English.
Additionally to $|V| \in \{10,16\}$, also a vocabulary size of $|V|=50$ now helps the agents to create more resembling languages.
Only the language $L_{3,100}$ emerges, that utilizes a vocabulary of $|V|=100$, and it aligns similarly to English as $L_{3,50}$ and slightly worse than $L_{3,10}$.
...
However, while $Lang_{6,50}$ is the language with the highest alignment, it is one of only two languages that emerged with $n=6$.
This indicates that agents struggle with so few contsraints, but if they do converge to a language, it makes use of similar structures as English.
...
While most of the emerged languages align with the salience order 'Si > Sh > C', languages with the lowest loss align with the salience order 'Sh > Si > C'.
...
While high alignments with English can be achieved with any $n$, the normalized loss remains consistently low only with $n \geq 2$.
...
This is anyway less consistent.
When given a vocabulary with $|V|=2$, the emerged languages have almost always a relatively low alignment with English.
...
While the \emph{size} is almost always communicated as the most important attribute, it is never predicted correctly by the receiver, as the accuracy of the \emph{size} lies around 52\%.
The opposite applies to the \emph{color}.
...
Looking at the influence of the message length $n$, one can see that languages with $n=1$ tend to have the worst alignment with English referring expressions.
...
Increasing it further tends to lower the alignment slightly, but stays mostly still around 65\%.
...
As expected, the most of them align with salience orders that have the \emph{color} as its most important attribute.
...
However, that alignment in between 70\% to 83\% is relatively low compared to the alignments that reach below 50\% on the 'Dale' datasets.
...
For instance, the receiver could struggle with specific \emph{colors} or other underlying structural patterns.
This results in a different language and in the lower alignment to English referring expressions.

The languages with the lowest alignment have a medium-sized vocabulary of $|V| \in \{10,16\}$ and are relatively short with $n \in \{1,3\}$.
While the language $Lang_{6,2}$ emerges which uses a small vocabulary with long messages, the normalized loss is much higher.