...
\begin{abstract}
  ...
  One agent describes objects in 3D scenes using invented vocabulary; the other interprets references based on attributes like shape, colour, and size. Learning is guided by feedback from successful interactions.
...
Referring expressions require systems to map visual attributes onto linguistic descriptions that uniquely identify target objects and thus can be use to study symbol grounding.
...
<<<<<<< HEAD
In this paper we explore how agents based on artificial neural networks learn referential grounding of entities in images of 3-dimensional scenes through language games \citep{Clark1996,Bartlett2005,Kirby2008,Steels2009,Zaslavsky2018}.
One agent is describing the entities represented as features within bounding boxes of objects, inventing new vocabulary as necessary. The other agent learns to interpret the reference of symbols by identifying one of the bounding boxes based on object attributes such as shape, colour and size.
Both agents learn through the success of interaction which is communicated through feedback.
The objective we would like us to achieve is to model constraints on the model that enable learning language through communication.
The novelty of our work, compared with the previous work with this setup \citep{Kharitonov2019,Lazaridou2017}, consists the extension of the popular CLEVR dataset \citep{Johnson2017a} with new artificially generated 3-d scenes of objects. These can be referred to based on attributes such as \emph{shape}, \emph{colour} and \emph{size} and discriminated based on different overlaps of these attributes between the target and the distractor objects.
...
\section{Dataset}
% complete control to have emergent languages.
% For this research
We extend the original CLEVR framework \citep{Johnson2017a} to have more control over the generated scenes.\footnote{Reference to a git repository.} % \href{https://github.com/DominikKuenkele/MLT\_Master-Thesis\_clevr-dataset-gen}{https://github.com/DominikKuenkele/MLT\_Master-Thesis\_clevr-dataset-gen}}
By this, the objects in the generated images are controlled to have different human-recognisable attributes, namely the \emph{shape}, \emph{size} and \emph{colour}.
=======
Reserch in this area investigates how artificial agents can develop referential abilities through language games — interactive scenarios where communication protocols emerge from repeated coordination attempts \citep{Clark1996,Bartlett2005,Kirby2008,Steels2009,Kharitonov2019,Lazaridou2017}. 
...
Our work is  a study of how increasing the complexity of the scene and therefore the space of potential referential expressions to be learned affects learning through interraction of particular configurations of neural networks.
...
We extend the original CLEVR framework \citep{Johnson2017a} to have more control over the generated scenes.\footnote{\href{https://github.com/DominikKuenkele/MLT\_Master-Thesis\_clevr-dataset-gen}{Source code on GitHub}}
...
>>>>>>> 31c67cc741c98b93cbb8d3851169571354be25a6
...
(i) the size of the generated images is 480x320 pixels;
...
\subsection{CLEVR colour}
The first generated dataset is called 'CLEVR colour', in which the target object is identifiable by just the color.
=======
...
Consider Figure \ref{fig:clevr-dale-5}, with the target object being the \emph{large purple cylinder}. The large purple sphere shares the size and colour, the two cubes only share the size, and the small turquoise sphere doesn't share any attribute.

\section{Method}
\subsection{Image processing}
\label{sec:image-processing}
The extract the features and process the images of the datasets, we build upon the proposed architecture in \citet{Johnson2017} which was used to train baseline models on the original CLEVR dataset.
...
The receivers' decoder LSTM processes the message symbol by symbol with a randomly initialised hidden state.
The received message sequence is processed symbol by symbol.
=======
This LSTM is then producing symbols until it generates an /texttt{<eos>} symbol.
...
<<<<<<< HEAD
As the discrete sampled categorical distribution of the message can't be differentiated, we use Gumbel-Softmax relaxation \citep{Jang2017} to turn it into a continuous distribution, thus allowing back-propagation.
=======
...
To solve the task correctly the sender is required to generate a referring expression about the target object through the discrete channel while the receiver needs to resolve it.
The experiment is set up in a way that avoids explicit human language information as human referring expressions or one-hot encoded attributes.
...
The sender is given a set of bounding boxes of all objects in the scene, whereas the target object is always the first bounding box and the distractors are shuffled.
The features of each bounding box are extracted using ResNet-101, combined and passed to an LSTM to produce a message.
...
The target area is located around the centre of the target object, consisting of 3$\times$3 regions.
...
The resulting matrix has 128$\times$14$\times$14 dimension, corresponding to the 14$\times$14 regions.

<<<<<<< HEAD
The sender's message is decoded using an LSTM with a hidden size $h_r=500$ and $e_r=100$.
The dot product is calculated between each encoded region of the image and the encoded message, and the $softmax$ function is applied subsequently, which results in a 14$\times$14 matrix.
This emphasises the correlation between the message and each region.
...
%TODO e? and h? and o?
% TODO params langauge games
The experiments are conducted with the following hyper-parameters: a learning rate of $2\times10^{-4}$, a temperature for the Gumbel-Softmax relaxation of 1 and \emph{Adam} \citep{Kingma2015} as optimiser.
...
This hypothesis is amplified by the results on the 'CLEVR colour' dataset.
...
<<<<<<< HEAD
        ...
        \subfigure['CLEVR colour' dataset with different $|V|$ highlighted]{
        ...
        \caption{Learning curves of all attention reference resolver games on each dataset.The colours correspond to different vocabulary sizes $|V|$. The baseline is marked in black.}
=======
        }
...
\begin{table}[ht]
        ...
        \begin{tabular}{cc|c|c|c}
                ...
                \bottomrule
        \end{tabular}
        \caption{Probability masses of the attention reference resolver after 128.000 games: $n$ are different maximum message lengths and $|V|$ are different vocabulary sizes. Results in red didn't pass the baseline. The results are sorted by the product of $n$ and $|V|$ which corresponds to available space for the message. The best results are achieved with a medium-sized messages space across all datasets.}
...
It reaches 62,16\% on the 'Dale-2' dataset, 49,61\% on the 'Dale-5' dataset and 41,68\% on the 'CLEVR colour' dataset.
...
The agents struggle the most on the 'CLEVR colour' dataset.
Hereby, only two configurations perform better than the baseline and reach a probability mass of around 64\% to 67\%.
Both utilise a medium message length of $n \in \{3,4\}$ and a medium-sized vocabulary of $|V| \in \{10,16\}$.
...
The same argument for a more difficult task when more objects are involved can be made for the 'CLEVR colour' dataset.
=======
...
Appendix \ref{app:attention-errors} shows examples of the wrongly identified regions on each dataset.
These are predictions by the agents that are wrong even though a language emerged successfully.
<<<<<<< HEAD
Main problems seemed to be target objects not being in the actual frame of the scene that the receiver was processing. % SD 2025-06-02 20:10:47 +0200: How could this happen? The scene has been trimmed, right?
=======
...
\subsection{Sender and receiver with narural language referring expressions}
...
\subsubsection{% Referring expression generation (sender)}
  Sender generating referring expressions}
...
Following, the model's success is validated on accuracy, recall and precision scores.
...
<<<<<<< HEAD
\subsubsection*{Results}
Table \ref{tab:results:bb-re-generator} shows the \emph{overall accuracy}, \emph{F1 scores} for each word.
...
With the 'Dale-2' dataset, the model can achieve perfect matches in 99\% of the cases in its best configuration.
Also the 'CLEVR colour' dataset allows the model to predict the correct referring expression in 93\% of the samples.
Opposed to that the model can only generate perfect referring expressions in 69\% of the samples of the 'Dale-5' dataset.
...
Since the overall results for the 'Dale-2' dataset are already close to perfect, the focus for this analysis lies on the remaining two datasets.
...
For the 'CLEVR colour' dataset, the scores are perfect.
This is not surprising, because all referring expressions for the 'CLEVR colour' dataset consist of exactly two attributes, shape and colour, and the first generated token will always be the only \emph{$<pad>$} token in the referring expression (corresponding to the unspecified size).
...
The model predicts the correct shape for all samples using the 'CLEVR colour' dataset, while both \emph{precision} and \emph{recall} lie around 98,3\% when using the 'Dale-5' dataset.
...
For the colour attribute, the metrics drop significantly for both 'Dale-5' and 'CLEVR colour' to an average of around 93\%.
Hereby, no meaningful difference can be seen across the datasets, but there are differences between the colours.
Some colours are predicted with \emph{precision} and \emph{recall} around 95\% to 96\%, while others are only around 90\%.
However, these differences are not reproducible across multiple runs and configurations.
The best and worst predicted colours vary and no conclusions can be drawn which colours are easier to predict for the model.

Finally, the size is the most difficult attribute to predict for the model.
Apart from the 'CLEVR colour' dataset, where a size never needs to be predicted and also is never predicted, the metrics for the prediction of size tokens are the lowest across all tokens.
They are the only mistakes, the model makes, when exposed to the 'Dale-2' dataset and the average \emph{precision} lies around 23\% below the average of predictions of the colour for the 'Dale-5' dataset, while the average \emph{recall} lies around 28,82\% below.
...
\begin{table*}[ht]
        ...
        \begin{tabular}{rr|cccccccc|c}
                \toprule
                                                 &             & {blue}  & {brown} & {cyan}  & {grey}  & {green} & {purple} & {red}   & {yellow} & \textbf{colour} \\\midrule
                ...
                \multirowcell{2}[0pt][r]{\textbf{CLEVR}                                                                                                           \\\textbf{colour}} & {Precision}           & {93,46} & {92,37} & {94,47} & {93,86} & {92,04} & {91,13}  & {90,07} & {94,7}   & \textbf{92,76} \\
                                                 & {Recall}    & {92,75} & {92}    & {95,98} & {89,92} & {94,12} & {91,13}  & {94,23} & {91,91}  & \textbf{92,76} \\
                \bottomrule
        \end{tabular}
        \caption{Precision and Recall in \% for colour tokens with $e=100$, $LSTM_o=500$ and $LSTM_e=30$. The column \textbf{colour} shows the average across all colours.}
...
While the shape is easily identified, the model has bigger problems to identify the colour and especially the size of the target object.
...
In the 'Dale-2' dataset, there are only two possibilities, while the four distractors in the 'Dale-5' dataset and up to nine distractors in the 'CLEVR colour' give the model a bigger choice.
...
For instance using the 'Dale-2' dataset, when only two random objects are placed in a scene, a second (or third) attribute to discriminate the objects is only needed, when the shape is the same.
Otherwise, the shape is enough, and the RE is only one word long.
The probability for this lies at 66,6\%.
For shape and colour being enough, the probability lies at 29,2\% and that all three attributes are necessary is 4,1\%.
Opposed to that the probabilities with four distractors as in the 'Dale-5' dataset are 19,8\%, 47\% and 33,2\% respectively.
...
This is verified by the lower precisions and recalls of the size and colour tokens.
...
\subsection{ %Referring expression resolution (receiver)}
  Receiver resolving referring expressions}
...
A central problem in the models' predictions is therefore the focusing on precise locations in the image and connecting them to the referring expression.
The \textbf{attention reference resolver} is meant to solve this problem in two ways.
First, the predictions must not be as precise, as only regions in a 14$\times$14 grid need to be identified.
...
When the \emph{colour} is the only discriminating attribute, the model achieves again similar results to the 'Dale-2' dataset, despite the higher number of distractors of up to 10.
...
\begin{table}[ht]
        ...
        \begin{tabular}{c|c}
                \toprule
<<<<<<< HEAD
                       & \multicolumn{1}{c}{\textbf{Dale-2}} & \multicolumn{1}{c}{\textbf{Dale-5}} & \multicolumn{1}{c}{\textbf{colour}} \\  \cmidrule(lr){2-2}\cmidrule(lr){3-3}\cmidrule(lr){4-4}
                $p$    & \textbf{$P$ mass}                   & \textbf{$P$ mass}                   & \textbf{$P$ mass}                  \\\midrule
...
The results also indicate that humans and artificial neural networks have quite different learning biases and cases that facilitate learning for humans (e.g. pragmatic referring described in the Dale-Reiter algorithm) is difficult to learn for systems.
Experiments demonstrate that once we add such learning biases (e.g. modelling focused attention) learning becomes more successful.
...
This paper investigated how neural agents learn referential grounding through emergent communication versus natural language.
We conducted three complementary experiments: (1) language games with emergent communication, (2) natural language referring expression generation, and (3) natural language referring expression resolution.

Our language games revealed that agents can successfully develop communication protocols, achieving substantial performance gains over baselines (30-40\% improvement).
However, emergent communication faces constraints: medium-sized vocabularies (10-16 symbols) and message lengths (2-4 tokens) proved most effective.
Scene complexity significantly impacts learning, with simpler scenes ('Dale-2') enabling near-perfect communication (96\%) while complex scenes ('Dale-5' and'CLEVR color') challenged most configurations.
Especially grounding color in the emergent langauge provided the biggest difficulties.

The natural language experiments provided crucial insights into these limitations.
When generating referring expressions, models achieved high accuracy on simple scenes ('Dale-2': 99\%) but struggled with complex discriminations ('Dale-5': 69\%).
...
This suggests that even with pre-grounded color terms, the model struggles to reliably identify and discriminate colors in visual scenes, which explains why agents face significant challenges when trying to establish color grounding from scratch in emergent communication.
...
The image is encoded using a combination of ResNet-101 and several convolutional layers described in Section \ref{sec:image-processing}.
Bothe encodings are passed through a $tanh$ non-linearity, and the results are combined using a dot product.
...
\section{Examples of errors in attention prediction}
\label{app:attention-errors}
\begin{figure}[!htbp]
        \centering
        \subfigure['Dale-2']{
...
>>>>>>> 31c67cc741c98b93cbb8d3851169571354be25a6
        ...
        \caption{Examples of the \emph{attention reference resolvers'} predictions in games with successful communication with a probability mass lower than 50\% on the 'Dale' and 'CLEVR colour' datasets. The black rectangle shows the cropped section the model is actually seeing after the image is pre-processed. The green rectangle surrounds the target region that needs to be predicted while the red regions show the actual predictions of the model. The more intense the red, the higher is the probability that the model assigned to this region.}
