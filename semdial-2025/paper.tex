...
\maketitle
\begin{abstract}
  This paper explores how neural network-based agents learn to map continuous sensory input to discrete linguistic symbols through interactive language games.
  One agent describes objects in 3D scenes using invented vocabulary; the other interprets references based on attributes like shape, color, and size. Learning is guided by feedback from successful interactions.
...
Their sensors and actuators allow them to sample the world and their own state using measures that are continuous in nature such as intensity of light, distance, angles, velocity and others which can be measured with a high degree of accuracy.
...
How can agents achieve it through interactive grounding \citep{Regier1996,Roy2005,Cooper2023}?
...
The objective we would like us to achive is to model constraints on the model that enable learning language through communication.
...
Using these rules, we generate three datasets with the following contraints:
...
A sender neural model processes visual input.
% TODO neural model vs agent
...
As the discrete sampled categorical distribtion of the message can't be differentiated, we use Gumbel-Softmax relaxation \citep{Jang2017} to turn it into a continuous distribution, thus allowing backpropagation.
...
The same applies as well to the message length. % SD 2025-06-02 19:41:31 +0200: Add that is shown by different versions of the same coloured curve.
...
This happens due to center cropping the image to prepare it for the ResNet model. % SD 2025-06-02 20:10:47 +0200: How could this happen? The scene has been trimmed, right?
% DK 2025-06-03 10:00:00 +0200: (done)
...
\subsection{Sender and receiver on their own}
...
% TODO we know how natural langugae RE are groudnde in scense while we don't know this about emergend langauge RE
...
Also the 'CLEVR color' dataset allows the model to predict the correct referring expression in 93\% of the samples.
...
This is not surprising, because all referring expressions for the 'CLEVR color' dataset consist of exactly two attributes, shape and color, and the first generated token will always be the only \emph{<pad>} token in the referring expression (corresponding to the unspecified size).
...
In the 'Dale-2' dataset, there are only two possibilities, while the four distractors in the 'Dale-5' dataset and up to nine distractors in the 'CLEVR color' give the model a bigger choice.
...
This paper explored how artificial neural networks can learn referential grounding of entities in 3D scenes through language games. We extended the CLEVR dataset with new artificially generated scenes, allowing for more controlled variations in object attributes and their overlaps. Our experiments systematically compared different setups of language games using the EGG framework.

In the central experiment, where two agents solved a referring task, we observed that agents were able to learn to communicate and significantly boost their performance over a baseline of random messages. This boost was more pronounced on simpler datasets like 'Dale-2' (around 40\% points) compared to more complex ones like 'Dale-5' (around 30\% points) and 'CLEVR color' (less consistent, with only two configurations beating the baseline). We found that smaller vocabulary sizes and message lengths generally led to faster learning, although very small vocabularies could also be too restrictive. The 'Dale-5' and 'CLEVR color' datasets posed greater challenges, likely due to the increased number of distractors and the need to encode more complex information for unique object identification. Errors in attention prediction often involved the receiver being uncertain between the target and distractors sharing multiple attributes.

Two additional experiments investigated the sender and receiver models in isolation, introducing explicit natural language bias. For the referring expression generation (sender) task, the model successfully extracted discriminative features and produced referring expressions. Performance was highly dependent on the number of distractors, with the 'Dale-2' dataset yielding near-perfect accuracy (99\%), while 'Dale-5' and 'CLEVR color' showed lower accuracies (69\% and 93\% respectively). The model found it easier to identify shape than color, and especially size, which we attribute to the increasing complexity of discrimination with more distractors and the nature of the incremental GRE-algorithm used for generating referring expressions.

In the referring expression resolution (receiver) task, the models demonstrated a strong ability to understand natural language referring expressions and ground them in the target object within the image. High probability masses (over 90\% in most cases) were achieved across all datasets, indicating that the attention reference resolver effectively connected the encoded referring expressions with specific regions in the image. Even with more complex referring expressions and a higher number of distractors, the receiver models maintained high accuracy in focusing on the correct object.

In conclusion, our work demonstrates the efficacy of language games for learning referential grounding in visual scenes. While the complexity of the scene and the number of distractors significantly impact the learning process and the performance of the emergent language, the models show a robust ability to learn and utilize communicative signals. The insights gained from analyzing the sender and receiver models separately highlight the challenges in generating precise referring expressions in complex environments and the robustness of the attention mechanism in resolving them. Future work could explore more dynamic and interactive language games, as well as investigate the emergence of more compositional and human-like language structures.
